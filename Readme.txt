video resolution changed after concatenating with Pr
...
There are two files containing the code, the difference between them 
is the .obj file. You can run all the features of cornell-box in folder 'cw' 
and check Phong shading in the 'sphere' folder. Both of them can be 
compiled by make.
...
After compile, the model will rotate automatically.
you may want to change line 743 'bool draw = true' to 'bool draw = false' or press 'h'
and use key 'm' and 'r' to render the model step by step.
'sphere' only have key control on camera and light.
